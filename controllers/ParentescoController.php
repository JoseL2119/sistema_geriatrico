        ...
        switch($requestMethod){
            ...
            case 'GET':
                if($id==0){
                    ...
                    $response = Parentesco::getAll();
                    echo json_encode($response);
                }
                else{
                    // $response = ExcavacionCdPiar::getById($id);
                    $response = Parentesco::getById($id);
                    echo json_encode($response);
                }
                break;
            ...
            case 'POST':
                ...
                $response = Parentesco::insert($data);
                echo json_encode($response);
                break;
            ...
            case 'PUT':
                $data = array();
                parse_str(file_get_contents('php://input'), $data);
                $response = Parentesco::update($id, $data);
                echo json_encode($response);
                break;

            case 'DELETE':
                $response = Parentesco::delete($id);
                echo json_encode($response);
                break;
