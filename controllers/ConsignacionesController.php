        ...
        switch($requestMethod){
            ...
            case 'GET':
                if($id==0){
                    ...
                    $response = Consignaciones::getAll();
                    echo json_encode($response);
                }
                else{
                    // $response = ExcavacionCdPiar::getById($id);
                    $response = Consignaciones::getById($id);
                    echo json_encode($response);
                }
                break;
            ...
            case 'POST':
                ...
                $response = Consignaciones::insert($data);
                echo json_encode($response);
                break;
            ...
            case 'PUT':
                $data = array();
                parse_str(file_get_contents('php://input'), $data);
                $response = Consignaciones::update($id, $data);
                echo json_encode($response);
                break;

            case 'DELETE':
                $response = Consignaciones::delete($id);
                echo json_encode($response);
                break;
