...
require_once "./models/Operadores.php";
        ...
        switch($requestMethod){
            ...
            case 'GET':
                if($id==0){
                    ...
                    $response = Operadores::getAll();
                    echo json_encode($response);
                }
                else{
                    // $response = ExcavacionCdPiar::getById($id);
                    $response = Operadores::getById($id);
                    echo json_encode($response);
                }
                break;
            ...
            case 'POST':
                ...
                $response = Operadores::insert($data);
                echo json_encode($response);
                break;
            ...
            case 'PUT':
                $data = array();
                parse_str(file_get_contents('php://input'), $data);
                $response = Operadores::update($id, $data);
                echo json_encode($response);
                break;

            case 'DELETE':
                $response = Operadores::delete($id);
                echo json_encode($response);
                break;
