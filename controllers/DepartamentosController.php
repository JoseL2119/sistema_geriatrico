        ...
        switch($requestMethod){
            ...
            case 'GET':
                if($id==0){
                    ...
                    $response = Departamentos::getAll();
                    echo json_encode($response);
                }
                else{
                    // $response = ExcavacionCdPiar::getById($id);
                    $response = Departamentos::getById($id);
                    echo json_encode($response);
                }
                break;
            ...
            case 'POST':
                ...
                $response = Departamentos::insert($data);
                echo json_encode($response);
                break;
            ...
            case 'PUT':
                $data = array();
                parse_str(file_get_contents('php://input'), $data);
                $response = Departamentos::update($id, $data);
                echo json_encode($response);
                break;

            case 'DELETE':
                $response = Departamentos::delete($id);
                echo json_encode($response);
                break;
