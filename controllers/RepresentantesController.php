...
require_once "./models/Representantes.php";
        ...
        switch($requestMethod){
            ...
            case 'GET':
                if($id==0){
                    ...
                    $response = Representantes::getAll();
                    echo json_encode($response);
                }
                else{
                    // $response = ExcavacionCdPiar::getById($id);
                    $response = Representantes::getById($id);
                    echo json_encode($response);
                }
                break;
            ...
            case 'POST':
                ...
                $response = Representantes::insert($data);
                echo json_encode($response);
                break;
            ...
            case 'PUT':
                $data = array();
                parse_str(file_get_contents('php://input'), $data);
                $response = Representantes::update($id, $data);
                echo json_encode($response);
                break;

            case 'DELETE':
                $response = Representantes::delete($id);
                echo json_encode($response);
                break;
