...
require_once '../../models/TiemposParada.php'; // Ajusta la ruta según tu estructura
...
if (!empty($filtros['fecha_inicio']) && !empty($filtros['fecha_fin'])) {
    $datos = TiemposParada::getFiltrados($filtros);
} else {
    $datos = TiemposParada::getAll();
}
...
<head>
    <meta charset="utf-8">
    <title>Tiempos de Parada - Reporte PDF</title>
    <style>
        /* Estilos generales */
        body { 
...
<body>
    ...
    <h2 style="text-align: center; margin-bottom: 20px;">Registros de Tiempos de Parada</h2>
    ...
    <table class="table">
        <thead>
            <tr>
                <th>Fecha</th>
                <th>Tiempo total MOBO 1</th>
                <th>Tiempo total MOBO 2</th>
                <th>Tiempo total MOBO 3</th>
                <th>Tiempo total de Parada de la cinta</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($datos as $item): 
                $total1 += $item->total_tiempo_parada_1;
                $total2 += $item->total_tiempo_parada_2;
                $total3 += $item->total_tiempo_parada_3;
                $totalC += $item->total_minutos_parada_cinta;
                $cont ++;

                ?>
                <tr>
                    <td class="text-center"><?= date_format(date_create($item->fecha), 'd-m-Y') ?></td>
                    <td class="text-center"><?= $item->total_tiempo_parada_1 ?> min</td>
                    <td class="text-center"><?= $item->total_tiempo_parada_2 ?> min</td>
                    <td class="text-center"><?= $item->total_tiempo_parada_3 ?> min</td>
                    <td class="text-center"><?= $item->total_minutos_parada_cinta ?> min</td>
                </tr>
            <?php endforeach; ?>
            <tr>
                <td class="text-center">Totales</td>
                <td class="text-center"><?php echo number_format($total1/60, 2) ?> h</td>
                <td class="text-center"><?php echo number_format($total2/60, 2) ?> h</td>
                <td class="text-center"><?php echo number_format($total3/60, 2) ?> h</td>
                <td class="text-center"><?php echo number_format($totalC/60, 2) ?> h</td>
            </tr>
        </tbody>
    </table>
